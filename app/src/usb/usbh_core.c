...
USBH_StatusTypeDef USBH_Init(USBH_HandleTypeDef *phost,
    ...
    /* Create USB Host Task */
    USBH_Thread_Atrr.name       = "usb/usbh_Queue";
 ...
 */
uint8_t
USBH_FindInterface(USBH_HandleTypeDef *phost, uint8_t Class, uint8_t SubClass, uint8_t Protocol) {
 ...
 */
uint8_t
USBH_FindInterfaceIndex(USBH_HandleTypeDef *phost, uint8_t interface_number, uint8_t alt_settings) {
    USBH_InterfaceDescTypeDef *pif;
    USBH_CfgDescTypeDef       *pcfg;
    uint8_t                    if_ix = 0U;
...
USBH_StatusTypeDef USBH_Process(USBH_HandleTypeDef *phost) {
    ...
    switch (phost->gState) {
        ...
        case HOST_SET_CONFIGURATION:
            /* set configuration */
            if (USBH_SetCfg(phost, (uint16_t)phost->device.CfgDesc.bConfigurationValue)
                == USBH_OK) {
    ...
    switch (phost->EnumState) {
        ...
        case ENUM_GET_MFC_STRING_DESC:
            if (phost->device.DevDesc.iManufacturer != 0U) {
                ...
                ReqStatus = USBH_Get_StringDesc(
                    phost, phost->device.DevDesc.iManufacturer, phost->device.Data, 0xFFU);
                if (ReqStatus == USBH_OK) {
        ...
        case ENUM_GET_PRODUCT_STRING_DESC:
            if (phost->device.DevDesc.iProduct != 0U) {
                ...
                ReqStatus = USBH_Get_StringDesc(
                    phost, phost->device.DevDesc.iProduct, phost->device.Data, 0xFFU);
                if (ReqStatus == USBH_OK) {
        ...
        case ENUM_GET_SERIALNUM_STRING_DESC:
            if (phost->device.DevDesc.iSerialNumber != 0U) {
                ...
                ReqStatus = USBH_Get_StringDesc(
                    phost, phost->device.DevDesc.iSerialNumber, phost->device.Data, 0xFFU);
                if (ReqStatus == USBH_OK) {

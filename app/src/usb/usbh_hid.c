...
static USBH_StatusTypeDef USBH_HID_ClassRequest(USBH_HandleTypeDef *phost) {
    ...
    switch (HID_Handle->ctl_state) {
        ...
        case HID_REQ_GET_REPORT_DESC:
            ...
            /* Get Report Desc */
            classReqStatus =
                USBH_HID_GetHIDReportDescriptor(phost, HID_Handle->HID_Desc.wItemLength);
...
static USBH_StatusTypeDef USBH_HID_Process(USBH_HandleTypeDef *phost) {
    ...
    switch (HID_Handle->state) {
        ...
        case HID_IDLE:
            status = USBH_HID_GetReport(
                phost, 0x01U, 0U, HID_Handle->pData, (uint8_t)HID_Handle->length);
            if (status == USBH_OK) {
        ...
        case HID_GET_DATA:
            USBH_InterruptReceiveData(
                phost, HID_Handle->pData, (uint8_t)HID_Handle->length, HID_Handle->InPipe);

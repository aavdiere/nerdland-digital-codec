...
static USBH_StatusTypeDef USBH_HandleControl(USBH_HandleTypeDef *phost) {
    ...
    switch (phost->Control.state) {
        case CTRL_SETUP:
            /* send a SETUP packet */
            USBH_CtlSendSetup(
                phost, (uint8_t *)(void *)phost->Control.setup.d8, phost->Control.pipe_out);
        ...
        case CTRL_DATA_IN:
            ...
            USBH_CtlReceiveData(
                phost, phost->Control.buff, phost->Control.length, phost->Control.pipe_in);
        ...
        case CTRL_DATA_OUT:
            ...
            USBH_CtlSendData(
                phost, phost->Control.buff, phost->Control.length, phost->Control.pipe_out, 1U);

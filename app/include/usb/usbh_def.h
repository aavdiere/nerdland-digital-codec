...
#define LE24(addr) \
    (((uint32_t)(addr)[0]) | (((uint32_t)(addr)[1]) << 8) | (((uint32_t)(addr)[2]) << 16))
...
#define LE64(addr)                                                                        \
    (((uint64_t)(addr)[0]) | (((uint64_t)(addr)[1]) << 8) | (((uint64_t)(addr)[2]) << 16) \
...
#define USB_DESC_OTHER_SPEED_CONFIGURATION \
    ((USB_DESC_TYPE_OTHER_SPEED_CONFIGURATION << 8) & 0xFF00U)
...
typedef struct _EndpointDescriptor {
    ...
    uint16_t
        wMaxPacketSize; /* Maximum Packet Size this endpoint is capable of sending or receiving */
...
typedef struct _ConfigurationDescriptor {
    ...
    uint8_t
        bmAttributes; /* D7 Bus Powered , D6 Self Powered, D5 Remote Wakeup , D4..0 Reserved (0)*/
